...
const Card = ({ color, title, flag }) => {
  ...
  const getTodosLength = () => {
    return todosDoFirstLength + todosScheduleLength + todosDelegateLength + todosDontDoLength;
  };

  const getTodos = () => {
    return todosDoFirst.concat(todosSchedule).concat(todosDelegate).concat(todosDontDo);
  };

  const getTodosDoFirst = () => {
    return todosDoFirst;
  };

  const getTodosSchedule = () => {
    return todosSchedule;
  };

  const getTodosDelegate = () => {
    return todosDelegate;
  };

  const getTodosDontDo = () => {
    return todosDontDo;
  };

  const getTodosDoFirstLength = () => {
    return todosDoFirstLength;
  };

  const getTodosScheduleLength = () => {
    return todosScheduleLength;
  };

  const getTodosDelegateLength = () => {
    return todosDelegateLength;
  };

  const getTodosDontDoLength = () => {
    return todosDontDoLength;
  };
